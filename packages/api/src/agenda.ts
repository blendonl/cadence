...
export type AgendaItemCreateInput = AgendaItemCreateRequestDto & {
  type?: string;
  status?: string;
...
const normalizeAgendaItemType = (value?: string): string | undefined => {
  ...
  if (normalized === 'SUBTASK' || normalized === 'REGULAR' || normalized === 'TASK') return 'TASK';
  return undefined;
};
...
const resolveStartAt = (
  ...
  if (!scheduledTime || !DATE_ONLY_PATTERN.test(agendaId)) return undefined;
...
const normalizeCreateInput = (input: AgendaItemCreateInput) => {
...
export const createAgendaApi = (client: IApiClient) => ({
  async getAgendaSummaries(
    ...
    return client.request<AgendaFindAllResponse>(
  ...
  async getAgendaView(params: {
    ...
    return client.request<AgendaViewResponseDto>(
  ...
  async createAgendaItem(request: AgendaItemCreateInput): Promise<AgendaItemEnrichedDto> {
    const { agendaId, body } = normalizeCreateInput(request);
    return client.request<AgendaItemEnrichedDto>(agendaItemBasePath(agendaId), {
      method: 'POST',
  ...
  async updateAgendaItem(
    ...
    return client.request<AgendaItemEnrichedDto>(`${agendaItemBasePath(agendaId)}/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async deleteAgendaItem(agendaId: string, id: string): Promise<void> {
    await client.request(`${agendaItemBasePath(agendaId)}/${id}`, { method: 'DELETE' });
  },

  async completeAgendaItem(agendaId: string, id: string): Promise<AgendaItemEnrichedDto> {
    return client.request<AgendaItemEnrichedDto>(
      `${agendaItemBasePath(agendaId)}/${id}/complete`,
      { method: 'PUT' }
    );
  },
  ...
  async rescheduleAgendaItem(
    ...
    return client.request<AgendaItemEnrichedDto>(
      `${agendaItemBasePath(agendaId)}/${id}/reschedule`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ newDate, startAt, duration }),
      }
    );
  },
  ...
  async getAgendaItemById(
    ...
    return client.requestOrNull<AgendaItemEnrichedDto>(
      `${agendaItemBasePath(agendaId)}/${id}`
    );
  },

  async markAsUnfinished(agendaId: string, id: string): Promise<AgendaItemEnrichedDto> {
    return client.request<AgendaItemEnrichedDto>(
      `${agendaItemBasePath(agendaId)}/${id}/unfinished`,
      { method: 'POST' }
    );
  },
});
