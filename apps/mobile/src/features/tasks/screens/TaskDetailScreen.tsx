...
type TaskDetailScreenNavigationProp = StackNavigationProp<BoardStackParamList, 'TaskDetail'>;
type TaskDetailScreenRouteProp = RouteProp<BoardStackParamList, 'TaskDetail'>;
...
export default function TaskDetailScreen({ navigation, route }: Props) {
  const { boardId, taskId, columnId } = route.params;
    ...
    const loadData = async () => {
      try {
        ...
        if (!isCreateMode && currentTaskId) {
          ...
          setPriority(foundTask.priority || TaskPriority.LOW);
        }
      } catch (error) {
