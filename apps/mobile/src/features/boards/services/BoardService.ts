...
export class BoardService {
  constructor(
    ...
    @inject(COLUMN_REPOSITORY) private columnRepository: ColumnRepository,
    @inject(TASK_REPOSITORY) private taskRepository: TaskRepository,
    @inject(VALIDATION_SERVICE) private validator: ValidationService,
  ) {}
  ...
  async saveBoard(board: Board): Promise<void> {
    ...
    const project = await this.projectService.getProjectById(board.project_id);
  ...
  async removeColumnFromBoard(
    ...
    const deleted = await this.columnRepository.deleteColumn(
      board.id,
      columnId,
    );
    if (deleted) {
  ...
  async updateColumn(
    ...
    const updated = await this.columnRepository.updateColumn(
      board.id,
      columnId,
      updates,
    );
